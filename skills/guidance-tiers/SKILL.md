...
[`check-guidance.sh`](check-guidance.sh) enforces the tiers: a frozen file changed since the base ref fails, an added one passes, and the tier's index README changes only alongside the directory's contents; ADRs are numbered without gap or duplicate; every living document exists; optionally, every backticked name in one section of the guidance file resolves to something real. Copy it into the repository, edit the configuration block at its top, and run it from anywhere inside the repository; its header states the usage and the base-ref default. Have CI run it on every pull request — a rule is only a rule if something fails when it is broken.
