...
- **The ready label.** The ticket template applies `ready-for-agent`, the unclaimed state `to-tickets` leaves, and the lifecycle mover lifts the label the script's `--unclaimed-label` named when a PR opens for the ticket; a triage set that names it differently names it on that template's `labels:` line, as the template says, and passes the same name to the script — § Parameters.
